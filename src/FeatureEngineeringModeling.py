...
class FeatureEngineer:
    ...
    def create_features(self, transactions: pd.DataFrame, accounts: pd.DataFrame, 
                       alert_dates: Dict[str, int] = None) -> pd.DataFrame:
        """
        ...
        for idx, acct in enumerate(accounts['acct']):
            ...
            features.update(self._statistical_features(txn_as_sender, txn_as_receiver, all_txn))
            
            # B. Alert Behavioral Pattern Features
            features.update(self._behavioral_features(txn_as_sender, txn_as_receiver, all_txn))
            
            # Missing value features
            features.update(self._missing_value_features(txn_as_sender, txn_as_receiver))
    ...
    def _filter_by_alert_date(self, transactions: pd.DataFrame, 
                              alert_dates: Dict[str, int]) -> pd.DataFrame:
    ...
    def _statistical_features(self, txn_sender: pd.DataFrame, 
                             txn_receiver: pd.DataFrame, 
                             all_txn: pd.DataFrame) -> Dict:
        ...
        # Send/Receive Ratio
        features['send_receive_count_ratio'] = (
            features['count_outgoing'] / max(features['count_incoming'], 1)
        )
        
        # Transaction velocity (average daily count)
        if len(all_txn) > 0:
            date_range = all_txn['txn_date'].max() - all_txn['txn_date'].min() + 1
            features['transaction_velocity'] = features['count_total'] / max(date_range, 1)
            features['active_days'] = all_txn['txn_date'].nunique()
            features['days_since_first'] = all_txn['txn_date'].max() - all_txn['txn_date'].min()
        else:
        ...
        if len(txn_receiver) > 0:
            ...
            # Large/Small transaction counts
            features['count_large_incoming'] = (txn_receiver['txn_amt'] >= 1000000).sum()
        ...
        # Amount ratios
        features['incoming_outgoing_amt_ratio'] = (
            features['sum_incoming_amt'] / max(features['sum_outgoing_amt'], 1)
        )
        
        return features
    
    def _behavioral_features(self, txn_sender: pd.DataFrame, 
                            txn_receiver: pd.DataFrame, 
                            all_txn: pd.DataFrame) -> Dict:
        ...
        if len(all_txn) > 0:
            ...
            for days in [1, 3, 7]:
                ...
                features[f'pct_amt_last_{days}d'] = (
                    recent_txns['txn_amt'].sum() / max(all_txn['txn_amt'].sum(), 1) * 100
                )
            ...
            if len(last_7d_txns) > 0:
                ...
                features['velocity_change_ratio'] = (
                    recent_velocity / max(overall_velocity, 0.001)
                )
            else:
        ...
        if len(all_txn) > 0:
            ...
            features['pct_foreign_currency'] = len(foreign_curr_txns) / len(all_txn) * 100
    ...
    def _missing_value_features(self, txn_sender: pd.DataFrame, 
                                txn_receiver: pd.DataFrame) -> Dict:
    ...
    def _create_zero_features(self, acct: str) -> Dict:
        ...
        zero_features = [
            'count_outgoing', 'count_incoming', 'count_total',
            'unique_receivers', 'unique_senders', 'send_receive_count_ratio',
            'transaction_velocity', 'active_days', 'days_since_first',
            'sum_outgoing_amt', 'mean_outgoing_amt', 'median_outgoing_amt',
            'std_outgoing_amt', 'max_outgoing_amt', 'min_outgoing_amt',
            'skew_outgoing_amt', 'count_large_outgoing', 'count_small_outgoing',
            'sum_incoming_amt', 'mean_incoming_amt', 'median_incoming_amt',
            'std_incoming_amt', 'max_incoming_amt', 'min_incoming_amt',
            'skew_incoming_amt', 'count_large_incoming', 'count_small_incoming',
            'incoming_outgoing_amt_ratio', 'pct_txn_as_receiver',
            'count_last_1d', 'pct_count_last_1d', 'sum_amt_last_1d', 'pct_amt_last_1d',
            'count_last_3d', 'pct_count_last_3d', 'sum_amt_last_3d', 'pct_amt_last_3d',
            'count_last_7d', 'pct_count_last_7d', 'sum_amt_last_7d', 'pct_amt_last_7d',
            'velocity_change_ratio', 'fan_in', 'fan_out', 'fan_in_out_ratio',
            'is_hub_flag', 'pct_mobile_banking', 'channel_diversity',
            'pct_foreign_currency', 'currency_diversity',
            'unk_ratio_is_self_txn', 'unk_ratio_channel_type'
        ]
...
class AlertPredictor:
    ...
    def train_with_cv(self, X: pd.DataFrame, y: pd.Series, n_folds: int = 5) -> Dict:
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
            ...
            print(f"Val: {len(X_val)} samples, {y_val.sum()} positive ({y_val.mean()*100:.2f}%)")
        ...
        print(f"{'='*60}")
        print(f"Mean F1-Score: {np.mean(cv_results['fold_f1']):.4f} (+/- {np.std(cv_results['fold_f1']):.4f})")
        print(f"Mean Precision: {np.mean(cv_results['fold_precision']):.4f} (+/- {np.std(cv_results['fold_precision']):.4f})")
        print(f"Mean Recall: {np.mean(cv_results['fold_recall']):.4f} (+/- {np.std(cv_results['fold_recall']):.4f})")
        print(f"Mean Threshold: {np.mean(cv_results['fold_thresholds']):.4f} (+/- {np.std(cv_results['fold_thresholds']):.4f})")
        
        # Calculate OOF metrics
        self.best_threshold = np.mean(cv_results['fold_thresholds'])
        oof_pred_final = (cv_results['oof_probabilities'] >= self.best_threshold).astype(int)
...
def main():
    ...
    # Set paths
    data_dir = r"c:\Users\ken55\OneDrive\桌面\玉山aicup\esun-ai-competition-2025\data"
    output_dir = r"c:\Users\ken55\OneDrive\桌面\玉山aicup\esun-ai-competition-2025"
    ...
    all_accounts_from_txn = pd.concat([
        transactions['from_acct'],
        transactions['to_acct']
    ]).unique()
    ...
    print(f"  Alert accounts (label=1): {train_accounts['label'].sum()} ({train_accounts['label'].mean()*100:.2f}%)")
    print(f"  Normal accounts (label=0): {(train_accounts['label']==0).sum()} ({(1-train_accounts['label'].mean())*100:.2f}%)")
    ...
    print(f"Predicted alert accounts: {test_predictions.sum()} ({test_predictions.mean()*100:.2f}%)")
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write("This report presents the feature engineering and modeling approach for predicting ")
        f.write("alert accounts based on transaction history. The solution implements comprehensive ")
        f.write("statistical and behavioral features with LightGBM modeling and class imbalance handling.\n\n")
        
        f.write("## 1. Data Overview\n\n")
        f.write(f"- **Total Transactions:** {len(transactions):,}\n")
        f.write(f"- **Training Accounts:** {len(train_accounts):,}\n")
        f.write(f"- **Alert Accounts (Positive):** {train_accounts['label'].sum():,} ({train_accounts['label'].mean()*100:.3f}%)\n")
        f.write(f"- **Normal Accounts (Negative):** {(train_accounts['label']==0).sum():,} ({(1-train_accounts['label'].mean())*100:.3f}%)\n")
        f.write(f"- **Test Accounts:** {len(predict_df):,}\n\n")
        
        f.write("### Class Imbalance\n\n")
        f.write("The dataset exhibits severe class imbalance with only ~0.3-0.5% positive cases. ")
        ...
        f.write(f"Created **{len(feature_engineer.feature_names)} features** across three categories:\n\n")
        ...
        f.write("- Percentage of transactions as receiver (alert accounts typically >60%)\n\n")
        ...
        f.write("**To prevent data leakage:** For alert accounts, only transactions **before** ")
        f.write("the alert date were used for feature calculation. This ensures the model learns ")
        ...
        f.write(f"**Method:** 5-Fold Stratified Cross-Validation\n\n")
        f.write(f"**Performance Metrics:**\n\n")
        f.write(f"| Metric | Mean | Std |\n")
        f.write(f"|--------|------|-----|\n")
        f.write(f"| F1-Score | {cv_results['mean_cv_f1']:.4f} | {cv_results['std_cv_f1']:.4f} |\n")
        f.write(f"| OOF F1-Score | {cv_results['oof_f1']:.4f} | - |\n\n")
        
        f.write(f"**Optimal Threshold:** {cv_results['best_threshold']:.4f}\n\n")
        
        f.write("The threshold was tuned on each validation fold to maximize F1-score, ")
        f.write("then averaged across folds for final predictions.\n\n")
        
        f.write("### Per-Fold Results\n\n")
        f.write(f"| Fold | F1-Score | Precision | Recall | Threshold |\n")
        f.write(f"|------|----------|-----------|--------|-----------|\n")
        ...
        f.write(f"**Predicted Alert Accounts:** {test_predictions.sum():,} ({test_predictions.mean()*100:.2f}%)\n\n")
        f.write(f"**Output File:** `result.csv`\n\n")
        
        f.write("## 7. Key Insights & Recommendations\n\n")
        
        f.write("### What Worked Well\n\n")
        f.write("1. **Time-based filtering** prevented data leakage and improved model reliability\n")
        f.write("2. **Behavioral pattern features** captured alert-specific patterns effectively\n")
        f.write("3. **Threshold tuning** significantly improved F1-score vs. default 0.5 threshold\n")
        f.write("4. **Class imbalance handling** through scale_pos_weight was crucial\n\n")
        
        f.write("### Potential Improvements\n\n")
        f.write("1. **SMOTE or other sampling techniques** could be explored\n")
        f.write("2. **Feature interactions** (e.g., fan_in × recent_activity) may add value\n")
        f.write("3. **Ensemble methods** combining multiple models\n")
        f.write("4. **Graph-based features** analyzing transaction networks\n")
        f.write("5. **Time series features** capturing trends and patterns over time\n\n")
        
        f.write("## 8. Conclusion\n\n")
        f.write("The implemented solution successfully addresses the alert account prediction problem ")
        f.write("through comprehensive feature engineering and careful modeling. The approach handles ")
        f.write("severe class imbalance, prevents data leakage, and achieves reasonable performance ")
        ...
        f.write("---\n\n")
        f.write(f"*Report generated by FeatureEngineeringModeling.py*\n")
